...
async def test_get_animal_by_species_not_found(async_client: AsyncClient):
    ...
    assert response.json() == {
        "detail": "Species not found. Доступные виды животных: Корова, Коза, Овца"
    }
...
async def test_create_animal_gt_unsuccess(async_client: AsyncClient, test_user_token):
    ...
    assert (
        ...
        == "Input should be 'Корова', 'Коза' or 'Овца'"
    )
    assert "application/json" in response.headers["content-type"]
...
async def test_create_with_incorrect_token_unsuccess(
    async_client: AsyncClient, test_user_token
):
    token = test_user_token(username="test@test.com")
    response = await async_client.post(
        "/tests/",
...
async def test_delete_animal_gt_success(async_client: AsyncClient, test_user_token):
    ...
    put_response = await async_client.delete(
        f"/tests/{created['id']}", headers={"Authorization": f"Bearer {token}"}
    )
    assert put_response.status_code == 200
...
async def test_delete_animal_gt_unsuccess(async_client: AsyncClient, test_user_token):
    ...
    created = response.json()
    put_response = await async_client.delete(
        f"/tests/{created['id'] + 100}", headers={"Authorization": f"Bearer {token}"}
    )
